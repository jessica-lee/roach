function data = importORTelem_ip25_edited(filename)
...
%  has the units of raw integer output, as recorded from the ADXL345.
    ...
    %506364,100,100,1496,1409,0,-2,-2,0,0,0,0,0,6,0.000000,876,0,0.000000,-0.096000,0,0,0
    % Unpack the rows of D
    data.times = D(:,1);              % microseconds
    data.inputL = D(:,2);             % raw counts, max = vbat
    data.inputR = D(:,3);             % raw counts, max = vbat
    data.DCL = D(:,4) / 4000;                % PWM counts, max = 3999 (see AP for details)
    data.DCR = D(:,5) / 4000;                % PWM counts, max = 3999 (see AP for details)
%     data.DCC = D(:,6);                % PWM counts, max = 3999 (see AP for details)
%     data.DCD = D(:,7);                % PWM counts, max = 3999 (see AP for details)
    data.wx = D(:,6) * count2deg;     % degrees / second
    data.wy = D(:,7) * count2deg;     % degrees / second
    data.wz = D(:,8) * count2deg;     % degrees / second
    data.wzavg = D(:,9) * count2deg;  % degrees / second
    data.xlx = D(:,10);            % raw counts, converted to G's below
    data.xly = D(:,11);            % raw counts, converted to G's below
    data.xlz = D(:,12);            % raw counts, converted to G's below
    data.BEMFA = D(:,13);             % ADC counts
    data.BEMFB = D(:,14);             % ADC counts
    data.vbatt = D(:,16);             % ADC counts
